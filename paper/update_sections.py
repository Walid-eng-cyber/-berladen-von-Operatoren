...
# Look for the line with "% ---" before "section{Experimente" and continue to just before "section{Fazit}"
start_marker = "\\section{Experimente und Analyse}"
end_marker = "\\section{Fazit}"

if start_marker in content and end_marker in content:
    # Find positions - search for the actual line breaks around these markers
    start_pos = content.find("% " + "-" * 57 + "\n" + start_marker)
    if start_pos == -1:
        # Try alternative format
        start_pos = content.find("% ---\n" + start_marker)
    if start_pos == -1:
        # Try without the preceding comment
    ...
    # Replace the section
    new_content = content[:start_pos] + new_sections + "\n\n" + content[end_pos:]
